...
var DEMO =
{
	...
	Initialize : function () {
		...
		this.InitCommands();
			...
			// Access the variable in Stability class
			// if (window.stable) {
			// 	console.log('Accessing Stability class variable from demo.js:', window.stable.Gm);
			// } else {
			// 	console.error('Stability instance not found');
			// }
	...
	//	this.count=1;
		...
		// Update load 
		if (window.stable.Gm < 0 && this.count==0 ) {
			this.count=1;
			let animation = 0;
			let position = 300;
	...
	UpdateCommands : function UpdateCommands() {
		...
		this.ms_Commands.movements.speed = curSpeed + ( targetSpeed - curSpeed ) * 0.05;
		...
		this.ms_Commands.movements.angle = curAngle + ( targetAngle - curAngle ) * 0.05;
