 ...
 */ var DEMO = {
    ...
    InitializeScene: function InitializeScene() {
        ...
        loader.load("models/BlackPearl/BlackPearl.obj", "models/BlackPearl/BlackPearl.mtl", function(object) {
            ...
            if (object.children) for(child in object.children)object.children[child].material.side = THREE.DoubleSide;
    ...
    InitCommands: function InitCommands() {
        ...
        var keyHandler = function keyHandler(action) {
            return function(event) {
                var key = event.which;
                if (key >= LEFT && key <= DOWN) switch(key){
                    case UP:
                        DEMO.ms_Commands.states.up = action;
                        break;
                    case RIGHT:
                        DEMO.ms_Commands.states.right = action;
                        break;
                    case DOWN:
                        DEMO.ms_Commands.states.down = action;
                        break;
                    case LEFT:
                        DEMO.ms_Commands.states.left = action;
                        break;
                }
            };
        };
    ...
    LoadMountains: function LoadSkyBox() {
        ...
        var addMountain = function addMountain(size) {
            var moutains = new THREE.Mesh(new THREE.CylinderGeometry(size, size, 35000, 32, 1, true), mountainsMaterial);
        ...
        var cylinder = new THREE.Mesh(new THREE.CylinderGeometry(150000, 150000, 150000, 32, 1, true), new THREE.MeshBasicMaterial({
    ...
    LoadSkyBox: function LoadSkyBox() {
        ...
        this.ms_SkyBox = new THREE.Mesh(new THREE.BoxGeometry(450000, 450000, 450000), skyBoxMaterial);
        ...
        if (environmentParameter !== null) this.ms_Environment = environmentParameter;
    ...
    UpdateEnvironment: function UpdateEnvironment(key) {
        ...
        if (raining) this.ms_soundRain.play();
        else this.ms_soundRain.pause();
        var sources = [
        ...
        var loaded = 0;
        var loadTexture = function(i1) {
            imageLoader.load(sources[i1], function(image) {
                cubeMap.images[i1] = image;
                loaded++;
                if (loaded === 6) cubeMap.needsUpdate = true;
            });
        };
        for(var i1 = 0, il = sources.length; i1 < il; ++i1)loadTexture(i1);
    ...
    Update: function() {
        ...
        if (this.ms_Camera.position.y < 0.0) this.ms_Camera.position.y = 2.0;
        ...
        if (this.ms_Raining) {
            ...
            for(i in this.ms_RainGeometry.vertices){
                ...
                if (this.ms_RainGeometry.vertices[i].y < -50) this.ms_RainGeometry.vertices[i].y = 50;
            }
        ...
        if (this.ms_Raining) this.ms_Camera.add(this.ms_Rain);
        // Updade clouds
    ...
    UpdateCommands: function UpdateCommands() {
        ...
        // Update speed
        var targetSpeed = 0.0;
        if (states.up) targetSpeed = 1.0;
        else if (states.down) targetSpeed = -0.5;
        var curSpeed = this.ms_Commands.movements.speed;
        this.ms_Commands.movements.speed = curSpeed + (targetSpeed - curSpeed) * 0.02;
        // Update angle
        var targetAngle = 0.0;
        if (states.left) targetAngle = Math.PI * 0.005;
        else if (states.right) targetAngle = -Math.PI * 0.005;
        if (states.down) targetAngle *= -1;
